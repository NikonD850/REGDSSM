...
import numpy as np
import utils
from dataset_RGB import create_data_loader,DataLoaderTrain_npz,DataLoaderVal_npz,DataLoaderTest_npz
...
def main():
    ...
    ##data prepare
    test_files_dirs=sorted(os.listdir(os.path.join(opt.father_val_path, 'blur')))
    ...
    if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
        ...
        for val_file in test_files_dirs:
            ...
            val_dataset = DataLoaderTest_npz(opt.father_val_path, val_file,opt)
            ...
            for ii, data_val in enumerate(tqdm(val_loader), 0):
                ...
                with torch.no_grad():
                    ...
                    input_img = funct.pad(input_img, (0, padw, 0, padh), 'reflect')
                    input_event = funct.pad(input_event, (0,padw,0,padh), 'reflect')
